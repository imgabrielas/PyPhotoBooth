...
class PhotoBoothApp:
    ...
    def countdown(self, seconds: int, photo_number: int) -> None:
        ...
        while True:
            ...
            self.draw_text(
                canvas,
                f"Photo {photo_number} in {remaining}",
    ...
    def draw_save_button(self, canvas: np.ndarray) -> None:
        bw = 210
        bh = 48
        bx = self.margin + self.preview_w + self.gap + (self.panel_w - bw) // 2
        ...
        self.draw_text(canvas, "SAVE STRIP", (bx + 31, by + 32), 0.8, WHITE, 2)
